...
1) Double-click **run.bat** (Windows) to start a small local web server.
2) Open **http://127.0.0.1:8000/index.html** in your browser.

Why this is needed: the page uses JavaScript **ES modules** (imports). Browsers typically block them when opening the file directly via `file://`, so the local server avoids those restrictions.

Tip: if you edit JS constants and don’t see changes, do a hard refresh (**Ctrl+Shift+R**) or disable cache in DevTools → Network.
...
| Arena size | 500 × 500 |
| Number of balls | 50 |
| Collision distance | `≤ 2 × R` (touching disks) |
| Vision radius | `10 × R` |
...
A collision is defined geometrically: distance between centers ≤ 2 × R (touching disks).
...
- **Space** — freeze/unfreeze (toggle pause/resume)
...
When a significant collision occurs in the right field, the simulation pauses and automatically generates diagnostic snapshots for post-analysis.
...
- `src/` contains ES-module JavaScript (policy, simulation, UI)
- No build system
- No external backend
- Run via a local web server (e.g. `python -m http.server`) so browsers can load ES modules
